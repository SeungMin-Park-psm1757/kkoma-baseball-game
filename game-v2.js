...
const BASE_PATH = [
  { x: 350, y: 495, label: "타석" }, { x: 908, y: 342, label: "1루" }, { x: 480, y: 302, label: "2루" },
  { x: 52, y: 342, label: "3루" }, { x: 480, y: 498, label: "홈" }
];
const FIELDERS = [
  { label: "3루수", x: 170, y: 340, height: 72 }, { label: "유격수", x: 355, y: 304, height: 66 },
  { label: "2루수", x: 605, y: 304, height: 66 }, { label: "1루수", x: 805, y: 340, height: 72 },
  { label: "좌익수", x: 270, y: 252, height: 52 }, { label: "중견수", x: 480, y: 236, height: 48 },
  { label: "우익수", x: 690, y: 252, height: 52 }
];
...
const state = {
  ...
  message: "준비되면 공이 날아와요.", muted: false, token: 0, action: null, strikes: 0, fieldAction: null, foul: null, runningPlay: null
};
...
function resolveStrike(reason) {
  ...
  if (strikeOut) { state.outs += 1; state.strikes = 0; }
  setMessage(strikeOut ? `${reason}! 삼진 아웃이에요.` : `${reason}! 스트라이크 ${state.strikes}/3.`); tone(110, .18); updateHud(); continuePlay(750);
}
...
function resolveHit(delta) {
  ...
  state.phase = "flight"; setMessage(type === "홈런" ? "완벽해요! 담장 너머로 날아가요!" : type === "땅볼" ? "땅볼! 수비수가 달려와요!" : `좋아요! ${type} 타구예요.`); tone(type === "홈런" ? 700 : 520, .12);
}
...
function buildOutPlay(bases, doublePlay, character) {
  const next = [...bases]; const moves = [];
  bases.forEach((occupied, index) => {
    if (!occupied) return;
    if (index === 0 && doublePlay) { moves.push({ startNode: 1, endNode: 2, character: null }); next[0] = false; }
    else moves.push({ startNode: index + 1, endNode: index + 1, character: null });
  });
  moves.push({ startNode: 0, endNode: 1, character });
  return { bases: next, moves, outs: doublePlay ? 2 : 1 };
}
...
function finishFlight() {
  ...
  if (type === "땅볼") {
    const doublePlay = state.bases[0] && state.outs < 2 && Math.random() < levels[state.level].doublePlayChance;
    const result = buildOutPlay(state.bases, doublePlay, state.character); const duration = runningDuration(1);
    state.outs += result.outs; state.runningPlay = { start: now, duration, moves: result.moves, resultBases: result.bases, runs: 0, type: doublePlay ? "병살" : "땅볼 아웃", resultMessage: doublePlay ? "병살! 2루와 1루에서 아웃이에요." : "1루에서 아웃! 다음 타자를 준비해요.", committed: false };
    state.fieldAction = { kind: doublePlay ? "double" : "groundOut", start: now, target }; setMessage(doublePlay ? "땅볼! 2루 찍고 1루로 던져요!" : "땅볼! 1루 승부예요!"); tone(120, .16); updateHud(); continuePlay(duration + 650); return;
  }
  if (caught) {
    state.fieldAction = { kind: "catch", start: now, target }; state.outs += 1; setMessage(`${target.label}가 잡았어요! 플라이 아웃!`); tone(90, .18); updateHud(); continuePlay(1450); return;
  }
  ...
  state.runningPlay = { start: now, duration, moves: result.moves, resultBases: result.bases, runs: result.runs, type, committed: false };
...
function continuePlay(delay) {
  ...
  const next = () => { if (token !== state.token || state.screen !== "game") return; commitRunningPlay(); if (state.score >= levels[state.level].target) endGame(true); else if (state.outs >= 3) endGame(false); else nextPitch(); };
  setTimeout(() => { if (state.paused) state.resumeAction = next; else next(); }, delay);
}
...
function draw() {
  ...
  if (state.phase === "pitching" && state.pitch) {
    ...
    drawBall(480 + (CONTACT.x - 480) * progress + Math.sin(progress * Math.PI) * state.pitch.curve * 75, 290 + (CONTACT.y - 290) * progress, 7 + progress * 5);
...
function drawPitcher(now) {
  ...
  drawGroundShadow(480, 353, 33, .18); drawFrame("pitcher", frame, 480, 353, 78, 92);
}
...
function activeFielderIndex() {
  if (state.phase === "flight") return state.flight?.target.fielderIndex; if (state.phase === "resulting" && state.fieldAction) return state.fieldAction.target.fielderIndex; return -1;
}

function drawFielders(now) {
  const active = activeFielderIndex();
  [...FIELDERS].sort((a, b) => a.y - b.y).forEach((fielder) => {
    const index = FIELDERS.indexOf(fielder); if (index === active) return;
...
function drawBaseRunners(now) {
  ...
  for (const move of state.runningPlay.moves) {
    const point = runningPoint(move, progress), name = move.character === 2 ? "tori" : "runner", frame = name === "tori" ? 2 : 1;
    drawGroundShadow(point.x, point.y, 27, .18); drawFrame(name, frame, point.x, point.y, 70, 78, point.flip);
  }
  if (progress >= 1) { commitRunningPlay(); state.runningPlay = null; }
}
...
function drawFieldingResult(now) {
  ...
  if (action.kind === "double") {
    drawGroundShadow(at.x, at.y, 25, .18); drawFrame("pitcher", elapsed < 350 ? 0 : 4, at.x, at.y, 66, 82, target.fieldX < fielder.x);
    if (elapsed < 350) drawTag("땅볼 포구!", at.x, at.y + 18, "#10233f");
    else if (elapsed < 950) { drawTag("2루 아웃!", BASE_PATH[2].x, BASE_PATH[2].y + 20, "#10233f"); drawThrowBall({ x: at.x, y: at.y - 52 }, { x: BASE_PATH[2].x, y: BASE_PATH[2].y - 38 }, (elapsed - 350) / 600); }
    else { const progress = Math.min(1, (elapsed - 950) / 700); drawTag("1루도 아웃!", BASE_PATH[1].x - 30, BASE_PATH[1].y + 18, "#10233f"); drawThrowBall({ x: BASE_PATH[2].x, y: BASE_PATH[2].y - 38 }, { x: FIELDERS[3].x - 9, y: FIELDERS[3].y - 50 }, progress); }
    return;
  }
...
function runSelfCheck() {
  ...
  result = buildOutPlay([true, false, false], true, 0); console.assert(result.outs === 2 && !result.bases[0] && result.moves.length === 2, "1루 주자 병살은 두 명이 뛰고 2아웃이어야 합니다.");
  result = buildOutPlay([false, true, false], false, 0); console.assert(result.outs === 1 && result.bases[1] && result.moves.length === 2, "일반 땅볼 아웃은 기존 주자를 유지해야 합니다.");
